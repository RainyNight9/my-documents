...
发两张纸，写：css上下左右居中。js继承，绑事件，事件循环各种log的顺序，反转二叉树。
一面：过来问了问vue底层 浏览器渲染过程 https协议 http缓存 promise的知识。
二面：写了个Promise.all实现。问了node模型 webpack底层 jsbridge redux 还有项目。顺风车他们自己项目是vue node都写。所以这两个是重点
三面：手写eventbus，手写简单中间件，针对你写的代码问各种情况怎么处理。 然后就是问自己的项目，细节。
四面：问职业方向，自己优缺点，目标。
